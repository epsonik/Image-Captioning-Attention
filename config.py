...
class config:
    ...
    embed_path = os.path.join(base_path,
                              '/home/odra/Images/glove/glove.6B.300d.txt')  # only makes sense when `embed_pretrain = True`
    ...
    model_basename = 'show_tell_coco_emb_resnet'  # any name you want
    messages_file = os.path.join(base_path, output_path, 'logs/show_tell_coco_emb_resnet/messages.txt')

    # training parameters
    epochs = 100
    batch_size = 400
    pretrained_encoder = 'Resnet101'  # denseNet201 InceptionV3 Resnet101
    encoder_dim = 2048
    ...
    checkpoint = os.path.join(base_path, output_path, 'checkpoints/best_checkpoint_show_tell_coco_emb_resnet.pth.tar')  # path to load checkpoint, None if none
    # checkpoint = None
    ...
    log_dir = os.path.join(base_path, output_path,
                           'logs/show_tell_coco_emb_resnet/')  # folder for saving logs for tensorboard
