...
class AdaptiveAttentionEncoderRegnet16(nn.Module):
    """
    Implementation of the encoder proposed in paper [1].
    ...
    def __init__(
        ...
        super(AdaptiveAttentionEncoderRegnet16, self).__init__()
        self.CNN = Regnet16(encoded_image_size)
        self.avg_pool = nn.AvgPool2d(
            kernel_size=encoded_image_size,
            stride=encoded_image_size
        ...
        self.global_mapping = nn.Sequential(
            # nn.Dropout(0.5),
            nn.Linear(3024, embed_dim),
            nn.ReLU(),
            # nn.BatchNorm1d(embed_dim, momentum=0.01)
        )
        self.spatial_mapping = nn.Sequential(
            # nn.Dropout(0.5),
            nn.Linear(3024, decoder_dim),
            nn.ReLU(),
            # nn.BatchNorm1d(embed_dim, momentum=0.01)
        )
