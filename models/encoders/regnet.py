...
class AttentionEncoderRegnet32(nn.Module):
    """
    ...
    def __init__(self, encoded_image_size: int = 7) -> None:
        super(AttentionEncoderRegnet32, self).__init__()
        self.CNN = Regnet32(encoded_image_size)
...
class EncoderRegnet32(nn.Module):
    """
    Implementation of the encoder proposed in paper [1].
    ...
    def __init__(self, encoded_image_size: int = 7, embed_dim: int = 512) -> None:
        super(EncoderRegnet32, self).__init__()
        self.CNN = Regnet32(encoded_image_size)
...
class AdaptiveAttentionEncoderRegNet32(nn.Module):
    """
    Implementation of the encoder proposed in paper [1].
    ...
    def __init__(
        ...
        super(AdaptiveAttentionEncoderRegNet32, self).__init__()
        self.CNN = Regnet32(encoded_image_size)
        self.avg_pool = nn.AvgPool2d(
            kernel_size=encoded_image_size,
            stride=encoded_image_size
        ...
        self.global_mapping = nn.Sequential(
            # nn.Dropout(0.5),
            nn.Linear(3712, embed_dim),
            nn.ReLU(),
            # nn.BatchNorm1d(embed_dim, momentum=0.01)
        )
        self.spatial_mapping = nn.Sequential(
            # nn.Dropout(0.5),
            nn.Linear(3712, decoder_dim),
            nn.ReLU(),
            # nn.BatchNorm1d(embed_dim, momentum=0.01)
        )
